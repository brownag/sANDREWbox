...
getSurfaceProperty <- function(cc, prop) {
  surface.prop <- horizons(cc)[cc$chiid %in% intersectComponentHorizon(cc, getMineralSoilSurfaceDepth(cc)+1),prop]
}
...
c$surface.rf.high <- as.numeric(profileApply(c, getSurfaceProperty, 'fragvoltot_h', simplify = F))
c$surface.txclay.high <- as.numeric(profileApply(c, getSurfaceProperty, 'claytotal_h', simplify = F))
...
par(mfrow=c(2,1))
plot(factor(c$subclass_calc[which(as.numeric(c$nirrcapcl) > 1 & as.numeric(c$nirrcapcl) < 8)]), main="CA630 Land Capability Subclasses", sub="for soils in LCC classes 2 through 7")
plot(factor(c$unit_calc[which(as.numeric(c$nirrcapcl) > 1 & as.numeric(c$nirrcapcl) < 5)]),main="CA630 Land Capability Units", sub="for soils in LCC 2, 3 or 4")

plot(factor(c$nirrcapscl, levels = c("c","e","s","w")), ylim=c(0,350), main="CA630 LC Subclass Breakdown (LCC 2 to 7)", sub="NASIS values")
plot(factor(c$subclass_calc, levels = c("c","e","s","w")), ylim=c(0,350), sub="Calculated values")

plot(factor(c$nirrcapunit[which(as.numeric(c$nirrcapcl) > 1 & as.numeric(c$nirrcapcl) < 5)]),main="CA630 Land Capability Unit Breakdown (LCC 2 to 4)", sub="NASIS values")
plot(factor(c$unit_calc[which(as.numeric(c$nirrcapcl) > 1 & as.numeric(c$nirrcapcl) < 5)]), sub="Calculated")
...
dsof[dsof$nirrcapcl == 1 | dsof$nirrcapcl == 8,]$subclass_calc <- "NA"
dsof[dsof$nirrcapcl == 1 | dsof$nirrcapcl >= 5,]$unit_calc <- "NA"
dsof[dsof$irrcapcl == 1 | dsof$irrcapcl == 8,]$subclass_calc <- "NA"
dsof[dsof$irrcapcl == 1 | dsof$irrcapcl >= 5,]$unit_calc <- "NA"
